...
/**
 * 自定义实现公平锁，实现避免处于线程饥饿状态（starvation,slipped condition,fairness,missing signal，nested monitor lockout（嵌套管程锁死)问题的解决方案）
 * 
 * 出现线程starvation现象有以下几种产生原因：
 * 		1）高优先级的线程一直占用cup时间片，而低优先级的线程一直处于阻塞状态，这个时候低优先级的线程就是处于startvation状态
...
/**
 * 用于唤醒和等待在改QueueObject对象上的各个线程。为一个信号灯（Semaphore）。之所以使用信号灯控制线程而不是单独使用某个对象的内部锁直接控制线程的原因是：
 * 		使用某个对象的内部锁 this.notify()方法去唤醒等待的线程的操作如果发生在this.wait()方法调用之前，那么当前等待的线程就无法接收到notify方法发送的
 * 		唤醒信号即出现missing signal 的现象，从而导致等待的线程可能一直处于等待状态，而不能唤醒。
 * 使用信号灯（Semaphore）的话，通过在内部定义一个竞争条件（isNotified）。来控制当前线程的状态。此时，即使是在doWait()方法前调用doNotify()方法，当前在该
